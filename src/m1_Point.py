...
class Point(object):
    """Holds data for a 20 point(x and y) and their operations."""
    ...
    def move_to(self, x, y):
        self.dist_traveled += ((self.x - x) ** 2 + (self.y - y) ** 2) ** 0.5
        self.x = x
        self.y = y
        self.c = x
        self.d = y
    ...
    def move_by(self, dx, dy):
        ...
        self.x = self.x + dx
        self.y = self.y + dy
        self.dist_traveled += ((self.x - dx) ** 2 + (dy - self.y) ** 2) ** 0.5
...
#
# DONE (continued): Then implement a class called   Point that has O METHODS
